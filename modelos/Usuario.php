...
public function Insertar(Usuario $p) {
        ...
        try {
            ...
            $consulta = ('INSERT INTO "Usuario" ( "Usuario", "Contrasena", "Rol")
		 VALUES (?,?,?);');
    ...
    public function ValideOldPass($idIns, $ContrasenaAntes){
        ...
        $consulta1 = $this->pdo->prepare('SELECT "fk_idUsuario" FROM "notaria" where "idnotaria" = ?');
        ...
        $consulta = $this->pdo->prepare('SELECT Us."Contrasena" FROM "Usuario" Us join "notaria" Ins on Us."idUsuario" = Ins."fk_idUsuario" WHERE Ins."fk_idUsuario" = ? AND  Us."Contrasena" = ?');
    ...
    public function Validar($Usuario, $Contrasena) {
        $consulta = $this->pdo->prepare('SELECT * FROM "Usuario"
		 JOIN "notaria" ON "idUsuario"="fk_idUsuario" JOIN "sucursal" ON "notaria"."fk_idsucursal" = "sucursal"."idsucursal"
...
//validar contraseña encriptada
        ...
        if (!empty($resultado)) {
            ...
            $_SESSION['idsucursal'] = $resultado->idsucursal;




                if ($resultado->Rol == "Notaria") {
                    $_SESSION['Rol'] = "Notaria";
                    //echo "Notaria";
                } elseif ($resultado->Rol == "Informador") {
                    $_SESSION['Rol'] = "Informador";
                    //echo "Informador";
                }elseif ($resultado->Rol == "Gerente") {
                    ...
                    //echo "Administrador";
                } elseif ($resultado->Rol == "Supervisor") {
                    $_SESSION['Rol'] = "Supervisor";
                    //echo "Supervisor";
                }elseif ($resultado->Rol == "Inmobiliaria") {
                    $_SESSION['Rol'] = "Inmobiliaria";
                    //echo "Inmobiliaria";
                }else {
                    session_destroy();
